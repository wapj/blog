...
보통 게임을 시작할 때 주인공에게 기본 도구 몇가지를 쥐어주고 시작하게 됩니다.
비록 초반에만 사용하긴 하지만, 게임에 적응할 수 있도록 도와주는 아주 고마운 존재입니다.
또한 기본 스킬들은 초반뿐 아니라 중반 후반에도 계속 사용하는 경우도 많이 있지요.

이번 글에서는 파이썬을 좋은 기본도구 몇가지를 소개하려고 합니다.
거의 매일 사용하게 되는 녀석들이니 혹시나 사용하고 있지 않다면, 이번 기회에 한번 사용해보시는 것을 추천드립니다.
...
아마도 이 글을 보시는 분들은 모두 사용하시고 있을거라고 생각이됩니다. 그만큼 가장 기본이 되는 툴이고 스탠다드 라이브러리에 들어있기도 합니다.

pip를 사용하면 생각보다 많은 기능을 제공한는데요. 이번글은 소개글이니 제가 사용하는 기능 한가지만 소개하려고 합니다.

바로 `pip freeze` 라는 기능인데요. 해당 명령어를 사용해서 해당 파이썬 환경에 설치된 라이브러리들을 모두 확인할 수 있습니다.
...
패키지 매니저는 스탠다드 라이브러리를 소개해놓고, 테스팅에서는 unitest가 아닌 pytest 를 소개하는지 궁금하실 텐데요.
...
뭔가 어려워 보이는 말이 두개가 동시에 나왔는데요. 예제로 살펴보겠습니다.
...
`@pytest.fixture` 가 데커레이터이구요. 데커레이터가 붙은 녀석의 함수를 `test_my` 에 주입을 시켜주게 됩니다. 그러면 테스트 함수 전후의 처리를 하게 되는데. yield 전까지 함수 실행전에 실행하고 yield 후는 함수를 실행하고 실행하게 됩니다.
...
홀로 코딩을 하게 되는 경우에는 이러한 개인적 성향과 취향이 전혀 관계가 없고 코딩 스티일에 신경을 쓰지 않아도 되니 생산성 향상에 도움이 될 수도 있을것 같습니다.

하지만, 팀으로 일한다면 `팀의 코딩 스타일은 한가지 색이어야 한다` 라는 철학을 가진 `타협하지 않는 코드 포매터` 가 바로 [black](https://github.com/psf/black) 입니다.
...
개인적으로는 사용하시는 에디터나 githook 에 넣어서 자동으로 포매팅 하시길 추천드립니다. 이미 수많은 성공적인 파이썬 프로젝트에 사용하고 있으며 저는 pycharm에서 저장시마다 자동 실행 되도록 해두었습니다.

코드리뷰시에 코딩 스타일에 의한 논쟁은 `black` 을 도입하는 순간 끝나게 됩니다. black에서 포매팅 해주는 것이 마음에 들지 않을 수 있는데, 그것도 익숙해지면 편안해집니다. 꼭꼭 사용하시길 강추합니다.

## 파이썬 환경 관리툴 pyenv, virtualenv

> 윈도우에서는 pyenv 를 제대로 지원하지 않습니다. ㅠㅠ
...
네 이런 부분에 대한 고민을 하고 계신 다면 pyenv, virtualenv(venv) 가 답입니다.
...
어떤 버전이 있는지 모르시는 경우에는 --list 를 넣으시면 리스트가 많이 나오게 됩니다. 2020년 11월 1일 기준으로 python3.9.0 도 있네요~ 3.9.0 은 너무 신상이라 그나마 안정적인 3.8.6을 설치하는게 나을 것 같습니다.
...
pyenv 는 git 으로 업데이트하는 것이 가장 깔끔했습니다. pyenv-installer 로 설치해도 git으로 설치됩니다.
...
지금까지 pyenv 의 간단한 사용법에 대해서 알아 보았습니다. 혹시 사용하고 있지 않다면 이 또한 꼭 사용해보시길 추천드립니다!

윈도우 사용자분은 venv 를 사용하시길 추천드립니다~! 파이썬 버전은 여러개를 사용하기 힘들수 있지만, 가상환경은 사용할 수 있습니다.

## jupyter notebook

마지막으로 소개드릴 도구는 jupyter 입니다. 저는 ipython-notebook 으로 먼저 알게된 프로젝트인데요.

jupyter notebook 은 웹브라우저에서 파이썬 코드를 실행하고 그 실행 결과를 바로 확인 할 수 있는 도구입니다. jupyter notebook 에 대한 자료는

코드를 테스트 하기 위해서 python 쉘에서 테스트 하고 계시다면 jupyter 가 정말 좋은 선택입니다.
...
쥬피터는 간단하게 테스트 코드를 돌려볼때 너무나 좋은 툴이라고 생각합니다. 데이터 분석가이신 분들은 쥬피터로 일을 하는 경우도 있다고 하는데 저는 개발자라 그런 경지까지 이르지는 못했습니다.

쥬피터로 파이썬 뿐아니라 자바, 코틀린, 스칼라 등등도 사용할 수 있으니 알아두시면 삶이 1%는 더 편해질 거라 생각합니다.
